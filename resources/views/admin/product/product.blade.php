            ...
            <div class="card pd-20 pd-sm-40">
                ...
                <div class="table-wrapper">
                    <table id="datatable1" class="table display responsive nowrap dataTable no-footer dtr-inline" role="grid" aria-describedby="datatable1_info" style="width: 949px;">
                        <thead>
                        <tr role="row">
                            <th class="wd-2p-force">Code</th>
                            <th class="wd-5p sorting_asc" >Product name</th>
                            <th class="wd-5p sorting_asc">Image</th>
                            <th class="wd-5p sorting_asc">Brand</th>
                            <th class="wd-5p sorting_asc">Category</th>
                            <th class="wd-2">Quantity</th>
                            <th class="wd-1p sorting_asc">Status</th>
                        ...
                        @foreach($product as $row)
                            <tr id="rwo{{$row->id}}">
                                ...
                                <td>{{ $row->name }}</td>
                                <td>
                                    <img height="60px" width="60px"  src="{{asset('public/media/product/'.$row->image_one) }}">
                                </td>
                                ...
                                <td>
                                    ...
                                    @if($row->status==1)
                                        ...
                                        @else
                                            <a href="#" class="disabled btn btn-sm btn-danger"title="un-active">
                                                <i class="fa fa-thumbs-down"></i></a>
                                        @endif
                                        ...
                                        <a href="" id="status"title="active" route="{{route('admin.product.status')}}"
                                           model_id="{{$row->id}}"class="btn btn-sm btn-info"><i class="fa fa-thumbs-up"></i></a>
                                            @else
                                            <a href="#" title="active"class="disabled btn btn-sm btn-info"><i class="fa fa-thumbs-up"></i></a>
                                            @endif
                                    @endif
                                </td>
